export default class uniRequest {
  ...
  async get(url, data, nativeSettings) {
    this.method = "GET";
    try {
      this.tempSava = async () => await this.exec(url, data, nativeSettings);
      !this.requestOptions.isWarn && (await this.tempSava());
    } catch (e) {
      console.log(e);
      this.err();
  ...
  async post(url, data, nativeSettings) {
    this.method = "POST";
    try {
      this.tempSava = () => this.exec(url, data, nativeSettings);
      !this.requestOptions.isWarn && this.exec(url, data, nativeSettings);
    } catch (e) {
      console.log(e);
      this.err();
      this.responseInterceptor();
